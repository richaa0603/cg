 ...
 * Only the Similar Repositories search is active; all other AI domains are
 * commented out below (see extension panel — only that tab is shown).
 */

const AI_API_BASE = (import.meta.env.VITE_AI_API_BASE as string) ?? "http://localhost:8000";
...
export async function getSimilarProjects(requirement: string): Promise<SimilarProject[]> {
  const res = await fetch(`${AI_API_BASE}/search`, {
    method: "POST",
